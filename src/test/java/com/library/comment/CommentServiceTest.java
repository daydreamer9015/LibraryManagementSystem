...
public class CommentServiceTest {
    ...
    /*
        댓글 생성 정상 케이스 테스트
            - 게시글과 작성자가 존재할 때 댓글이 정상 생성되는가?
            - 생성된 댓글의 내용이 올바른지?
            - Repository의 save() 메서드가 정확히 1번 호출되는가?
     */
    @Test
    ...
    /*
        리플렉션을 사용하여 BaseEntity의 날짜 필드 설정
            - 리플렉션은 런타임에 클래스 구조를 조작할 수 있는 강력한 도구임
    ...
    private void setBaseEntityFields(Comment comment, LocalDateTime createdAt, LocalDateTime updatedAt) {
        try {
            ...
            throw new RuntimeException("BaseEntity에 필드가 존재하지 않습니다: " + e.getMessage());

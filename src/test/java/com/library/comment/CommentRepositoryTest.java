...
/*
    CommentRepository 테스트 클래스
        - 목적 : 댓글 repository의 데이터베이스 조회 메서드들이 올바르게 동작하는지 검증
        - 테스트 항목
            - findByBoardIdAndStatus: 게시글 아이디와 상태별 댓글 조회
            - countByBoardIdAndStatus: 게시글 아이디와 상태별 댓글 개수 조회
        - @AutoConfigureTestDatabase(replace = AutoConfigureTestDatabase.Replace.ANY)
            - application.yml에 설정된 DB를 무시하고 테스트용 embedded database로 교체함
...
public class CommentRepositoryTest {
    @Autowired
    private CommentRepository commentRepository;    // 댓글 repo
    @Autowired
    private BoardRepository boardRepository;    // 게시글 repo (댓글은 게시판에 종속)
    @Autowired
    private MemberRepository memberRepository;  // 회원 repo (댓글과 게시판은 작성자 필요)
    ...
    void findByBoardIdAndStatus_활성댓글조회() {
        ...
        assertThat(activeComments.get(0).getContent()).isEqualTo("첫번째 댓글");     // 댓글 내용이 맞는지
        assertThat(activeComments.get(0).getStatus()).isEqualTo(CommentStatus.ACTIVE);     // 활성화 된 댓글이 맞는지
    }
    /*
        테스트 목적 : 특정 게시글의 활성 상태 댓글 개수를 정확히 계산하는지 검증
            - 점검 사항
                - ACTIVE 상태의 댓글만 카운트되는가
                - DELETED 상대의 댓글은 카운트에서 제외되는가
                - 반환되는 개수가 정확한가
     */
    @Test
    void countByBoardIdAndStatus_댓글개수조회() {
        ...
        for (int i =1; i <= 3; i++) {
            Comment comment = Comment.builder()
                    ...
                    .board(testBoard)        // 같은 게시글에 달린 3개의 댓글
        ...
        // When : 실제 테스트 - 특정 게시물의 활성 댓글 개수 조회
        ...
        assertThat(activeCount).isEqualTo(3L);  // 개수가 정확히 3개인지
    }
}

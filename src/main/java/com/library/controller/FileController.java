...
public class FileController {
    ...
    public ResponseEntity<Resource> downloadFile(@PathVariable Long fileId) {
        ...
        // 2. 물리적 파일 다운로드
        Resource resource = fileStorageService.loadFileAsResource(      // 서버 저장소에서 실제 파일을 Resource 객체로 로드
                boardFile.getFilePath(),        // 파일이 저장된 디렉토리 경로
                boardFile.getStoredFilename()); // 서버에 저장된 고유 파일명 (UUID + 촥장자)
        // 3. 다운로드 횟수 증가
        boardFile.increaseDownloadCount();  // 다운로드 횟수 1 증가 (메모리 상에서만 증가)
        // @Transactional로 인해 메서드 종료 시 더티체킹으로 자동 UPDATE 쿼리 실행됨

        // 4. 파일명 인코딩 (한글 파일명 처리)
        String encodedFilename;     // 인코딩된 파일명을 저장할 변수
        try {
            encodedFilename =       // 원본 파일명을 UTF-8로 URL 인코딩함 (한글 깨짐 방지)
        ...
        return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + encodedFilename + "\"")    // 브라우저가 파일을 다운로드하도록 지시함 (attachment) + 다운로드될 파일명 지정

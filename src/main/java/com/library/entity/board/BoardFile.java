...
// 게시글 첨부파일 Entity - 게시글에 첨부된 파일 정보를 관리하여, Board와 다대일 (N:1) 연관관계를 가짐
@Entity
...
public class BoardFile extends BaseEntity {
    ...
    // 소속 게시글 (Board와 N:1 관계) - 게시글 삭제 시 첨부파일도 함께 삭제됨
    ...
    @Column(nullable = false, length = 255)
    private String storedFilename;      // 저장된 파일명 (UUID로 저장 - 파일명 중복 방지하기 위해서)
    ...
    @Column(length = 10)
    private String fileExtension;       // 파일 확장자 (소문자) - 예) "pdf", "jpg", "png"

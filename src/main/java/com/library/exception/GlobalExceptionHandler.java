...
/*
    ...
    예외 처리 우선순위 (구체적인 것이 우선순위가 높음 => 일반적인 것이 우선순위 가장 낮음)
        1) InvalidFileException (파일 검증 실패 시)
        2) MaxUploadSizeExceededException (Spring 파일 크기 제한 넘었을 때 발생 시)
...
public class GlobalExceptionHandler {
    ...
    public ModelAndView handleMaxUploadSizeExceededException(MaxUploadSizeExceededException e) {
        ...
        mav.addObject("errorMessage", "업로드 가능한 최대 파일 크기는 10MB 입니다.");
    ...
    public ModelAndView handlerException(Exception e) {
        ...
        ModelAndView mav = new ModelAndView("error/file-error");
        mav.addObject("errorTitle", "시스템 오류 발생");

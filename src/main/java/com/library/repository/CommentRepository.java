...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query("SELECT c FROM Comment c " +     // Comment 엔티티 조회 (별칭: c)
            ...
            "ORDER BY c.createdAt ASC")     // 생성일시 오름차순 정렬 (오래된 댓글 이후에 최신 댓글 나옴)
    List<Comment> findByBoardIdAndStatus (@Param("boardId") Long boardId, @Param("status") CommentStatus status);   // @Param : JPQL의 :boardId, :status와 매핑
